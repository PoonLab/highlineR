...
test_that("sessions initate correctly with or without file import", {
  ...
  result <- init_session("test_sess")
  expect_equal(expected_session, result)
  ...
  expect_equal(expected_session, import_raw_seq(test_filename, session = "test_sess_2"))
  ...
  # import directory
  expect_error(import_raw_seq(paste0(base, "valid/fasta/"), session = "test_sess_2"),
                 NA)
...
test_that("sessions can be initialised and closed", {
  ...
  # initiate session
  expect_error(init_session("test_sess"),
               NA)
...
test_that("Data objects can be removed from sessions", {
  ...
  import_raw_seq(test_filename, session = "test_sess")
